...
function export_request_master_data_csv($type_csv_string, $type_file_name)
{
    ...
    $path = storage_path('app/public/INCOMING/MASTER_DATA_INCOMING/');
    if (!Storage::exists($path)) {
        Storage::makeDirectory('public/INCOMING/MASTER_DATA_INCOMING/');
    }
...
function export_request_po_data_csv($type_csv_string, $type_file_name)
{
    $export_csv_po_header[] = $type_csv_string;
    $export_csv_po_header[] = "U";
    $path = storage_path('app/public/INCOMING/UPDATE_DATA/');
    if (!Storage::exists($path)) {
        Storage::makeDirectory('public/INCOMING/UPDATE_DATA/');
    }
...
{
    $base_url = storage_path('app/public/OUTGOING/MASTERDATA/'.get_outgoing_folder_name($master_type)."/");
    $base_url_success = storage_path('app/public/OUTGOING/MASTERDATA_BACKUP/');
...
function generate_gr_csv($gr_header_id, $plant_code)
{
    ...
    $fp = fopen(storage_path('app/public/INCOMING/MIGO/'."MIGO_IN_".$plant_code."_GR_".str_pad($gr_header_id, 7, "0", STR_PAD_LEFT)."_".date("YmdHis").".csv"), 'w');
    fputcsv($fp, [
        "GR",
...
function generate_gi_csv($gi_header_id, $plant_code)
{
    ...
    if ($header_data["TR_GI_SAPHEADER_MVT_CODE"] == "101") {
        $fp = fopen(storage_path('app/public/INCOMING/MIGO/'."MIGO_IN_".$plant_code."_GR_".str_pad($gi_header_id, 7, "0", STR_PAD_LEFT)."_".date("YmdHis").".csv"), 'w');
    ...
    fputcsv($fp, [
        ...
        $fp = fopen(storage_path('app/public/INCOMING/MIGO/'."MIGO_IN_".$plant_code."_GI_".str_pad($gi_header_id, 7, "0", STR_PAD_LEFT)."_".date("YmdHis").".csv"), 'w');
...
function generate_tp_csv($tp_header_id, $plant_code)
{
    ...
    if ($header_data["TR_TP_HEADER_MVT_CODE"] == "Y21") {
        ...
        $fp = fopen(storage_path('app/public/INCOMING/MIGO/'."MIGO_IN_".$plant_code."_TP_".str_pad($tp_header_id, 7, "0", STR_PAD_LEFT)."_".date("YmdHis").".csv"), 'w');
        ...
        if ($header_data["TR_TP_HEADER_MVT_CODE"] == "551") {
            $fp = fopen(storage_path('app/public/INCOMING/MIGO/'."MIGO_IN_".$plant_code."_BIWA_".str_pad($tp_header_id, 7, "0", STR_PAD_LEFT)."_".date("YmdHis").".csv"), 'w');
...
function generate_cancellation_csv($cancellation_id, $plant_code)
{
    ...
    $fp = fopen(storage_path('app/public/INCOMING/MIGO/'."MIGO_IN_".$plant_code."_CANCEL_".str_pad($cancellation_id, 7, "0", STR_PAD_LEFT).date("YmdHis").".csv"), 'w');
...
function generate_stock_opname_csv($pid_id, $plant_code)
{
    ...
    $fp = fopen(storage_path('app/public/INCOMING/PID/'."PID_IN_".$plant_code."_".str_pad($pid_id, 7, "0", STR_PAD_LEFT)."_".date("YmdHis").".csv"), 'w');
