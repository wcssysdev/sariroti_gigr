...
class AddController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        foreach ($po_detail as $row) {
            ...
            for ($i = 0; $i < count($gi_materials); $i++) {
                if ($row["TR_PO_DETAIL_MATERIAL_CODE"] == $gi_materials[$i]["TR_GR_DETAIL_MATERIAL_CODE"]) {
                    $check_qty -= $gi_materials[$i]["TR_GR_DETAIL_LOCK_BOOKED_QTY"];
                }
            }
            if ($check_qty != 0) {
                return response()->json([
        ...
        $gi_id = std_insert_get_id([
            ...
            "data" => [
                ...
                "TR_GI_SAPHEADER_PSTG_DATE" => NULL,
                "TR_GI_SAPHEADER_DOC_DATE" => date("Y-m-d"),
                "TR_GI_SAPHEADER_BOL" => NULL,
                "TR_GI_SAPHEADER_TXT" => NULL,
                ...
                if ($po_detail_row["TR_PO_DETAIL_MATERIAL_CODE"] == $row["TR_GR_DETAIL_MATERIAL_CODE"]) {
                    ...
                    $gi_material_arr = array_merge($gi_material_arr, [
                        [
                            ...
                            "TR_GI_SAPDETAIL_PO_DETAIL_ID" => $po_detail_row["TR_PO_DETAIL_ID"],
                            "TR_GI_SAPDETAIL_SAPLINE_ID" => $count
                        ]
                    ]);
                    $count = $count + 2;
                }
